export function swiperAnimateCache() {
  const allBoxes:any = window.document.documentElement.querySelectorAll('.ani')
...
export function swiperAnimate(a:any) {
  clearSwiperAnimate()
  var b:any = a.slides[a.activeIndex].querySelectorAll('.ani')
...
export function clearSwiperAnimate() {
  var allBoxes:any = window.document.documentElement.querySelectorAll('.ani')
  for (var i = 0; i < allBoxes.length; i++) {
