<template>
  ...
  >
    ...
    >
      ...
      <!-- 选择框 -->
      <template v-if="judgeIsShow(formData,item)">
        <el-form-item
          :label="item.label"
          :prop="item.prop"
...
export default defineComponent({
  ...
  setup(prop, { emit }) {
    ...
    let judgeIsShow = (data: any, item: any) => {
      if (item.exceptVal) {
        return data[item.showProp]
          ? data[item.showProp] === item.exceptVal
            ? false
            : true
          : false;
      }
      if (data[item.showProp] === item.showVal) {
        return data[item.showProp]
          ? data[item.showProp] === item.showVal
            ? true
            : false
          : true;
      } else {
